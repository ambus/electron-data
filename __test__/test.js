(() => {
    ...
    describe('Save', () => {
        ...
        beforeEach(() => {
            electron_data.config({ filename: 'test.json' });
            return true;
        });
        ...
        test('Save with short file extension', () => {
            let desired_json = {
                "test-string": "test",
                "test-object": { "test": "test" },
                "test-array": ["test"],
            ...
            electron_data.config({ filename: 'test.jso' });

            return electron_data.save()
                .then(res => {
                    expect(res).toBe(undefined);

                    return fs.readFile(desired_config.path + '/test.jso', 'utf-8')
                        .then(content => expect(JSON.parse(content)).toMatchObject(desired_json));
                })
                .catch(err => expect(err).toBeNull());
        });
